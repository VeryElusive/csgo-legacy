...
void CChams::OnSceneEnd( ) {
	...
	for ( auto& player : queue ) {
		...
		if ( type == LOCAL && Config::Get<bool>( Vars.ChamDesync ) ) {
			...
			auto backupBones{ new matrix3x4_t[ 256 ] };
			std::memcpy( backupBones, player->m_CachedBoneData( ).Base( ), player->m_CachedBoneData( ).Count( ) * sizeof( matrix3x4_t ) );

			std::memcpy( player->m_CachedBoneData( ).Base( ), matrix, player->m_CachedBoneData( ).Count( ) * sizeof( matrix3x4_t ) );
			...
			std::memcpy( player->m_CachedBoneData( ).Base( ), backupBones, player->m_CachedBoneData( ).Count( ) * sizeof( matrix3x4_t ) );
