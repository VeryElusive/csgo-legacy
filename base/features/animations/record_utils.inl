...
FORCEINLINE void LagBackup_t::Apply( CBasePlayer* ent ) {
	...
	std::memcpy( ent->m_CachedBoneData( ).Base( ), this->m_matMatrix, ent->m_CachedBoneData( ).Count( ) * sizeof( matrix3x4_t ) );
}
...
FORCEINLINE LagBackup_t::LagBackup_t( CBasePlayer* ent ) {
	...
	memcpy( this->m_matMatrix, ent->m_CachedBoneData( ).Base( ), ent->m_CachedBoneData( ).Count( ) * sizeof( matrix3x4_t ) );
}