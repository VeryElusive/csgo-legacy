...
void CLogger::PrintToScreen( ) {
	...
	for ( auto& screenText : m_cOnscreenText ) {
		...
		Render::FilledRoundedBox( { 5, 22 * iterations - 3 }, { textSize.x + 9, 19 }, 90, 5, Menu::OutlineLight.Set<COLOR_A>( 255 * screenText.m_fAlpha ) );
		Render::FilledRoundedBox( { 6, 22 * iterations - 2 }, { textSize.x + 7, 17 }, 5, 5, Color( 10, 10, 10, static_cast< int >( 255 * screenText.m_fAlpha ) ) );
		Render::FilledRoundedBox( { 7, 22 * iterations - 1 }, { textSize.x + 5, 15 }, 5, 5, Menu::BackgroundCol.Set<COLOR_A>( 255 * screenText.m_fAlpha ) );
		Render::Line( { 5, 22 * iterations }, { 5, 22 * iterations - 3 + 17 }, ( iterations & 1 ? Menu::AccentCol : Menu::Accent2Col ).Set<COLOR_A>( 255 * screenText.m_fAlpha ) );
		//Render::Line( { 4, 22 * iterations - 3 }, { 4, 22 * iterations - 3 + 18 }, ( screenText.m_bAccent ? Menu::AccentCol : Menu::Accent2Col ).Set<COLOR_A>( 255 * screenText.m_fAlpha ) );
